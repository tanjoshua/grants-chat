...
export async function GET(req: Request) {
  ...
  const enableLinks = url.searchParams.get('enableLinks') === 'true';
  
  // If enableLinks is true, create a combined implementation
  if (enableLinks) {
    const linkText = url.searchParams.get('text') || 'Chat with AI';
    const linkClass = url.searchParams.get('class') || '';
    
    // Combined implementation with both button and links
    const script = `
(function() {
  // Check if widget already exists
  if (document.getElementById('ai-chat-combined-widget')) {
    console.log('Chat widget already exists, skipping initialization');
    return;
  }
  
  // Create a marker to prevent duplicate initialization
  const marker = document.createElement('div');
  marker.id = 'ai-chat-combined-widget';
  marker.style.display = 'none';
  document.body.appendChild(marker);

  // Add styles for both button and link widgets
  ...
  dialogIframe.className = 'ai-chat-dialog-iframe';
  dialogIframe.src = '${origin}/widget?embed=true&noheader=true&height=600px';
  
  content.appendChild(dialogIframe);
  dialog.appendChild(closeButton);
  dialog.appendChild(content);
  overlay.appendChild(dialog);
  document.body.appendChild(overlay);

  // Process all link placeholders
  ...
  closeButton.addEventListener('click', function() {
    ...
    try {
      dialogIframe.contentWindow.postMessage({ type: 'CHAT_WIDGET_CLOSE' }, '*');
    } catch (error) {
      console.error('Failed to communicate with chat iframe:', error);
    }
  });
  
  // Dialog outside click handler
  ...
  // Listen for close messages from both iframes
  ...
  // For backward compatibility, handle the button and link modes separately
  if (mode === 'button') {
    const script = `
(function() {
